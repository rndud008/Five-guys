...
(
  ...
  name  VARCHAR(100) NOT NULL COMMENT '지역이름',
  code  VARCHAR(100) NOT NULL COMMENT '코드번호',
  regId VARCHAR(100) NOT NULL COMMENT '중기예보 구역코드',
  PRIMARY KEY (id)
) COMMENT '지역(areacode)';
...
(
  ...
  infoname       VARCHAR(100) NULL     COMMENT '행사내용 만 가져옴 title 확인 ',
  infotext       TEXT         NULL     COMMENT '행사내용',
  PRIMARY KEY (id)
) COMMENT '여행 축제 행사내용';
...
(
  ...
  apiname   VARCHAR(100) NULL     COMMENT '호출한 정보 타이틀',
  regdate   DATETIME     NULL     DEFAULT now() COMMENT '호출 날짜',
  r_code    VARCHAR(100) NULL     COMMENT 'response code',
  r_message TEXT         NULL     COMMENT 'response message',
  PRIMARY KEY (id)
) COMMENT 'api 중복호출 방지';
...
(
  ...
  name        VARCHAR(100) NOT NULL COMMENT '시군구 이름',
  code        VARCHAR(100) NULL     COMMENT '코드번호',
  PRIMARY KEY (id)
) COMMENT '시군구(sigungucode)';

ALTER TABLE sigungucode
  ADD CONSTRAINT UQ_areacode_id UNIQUE (areacode_id);
...
(
  ...
  name           VARCHAR(100) NULL     COMMENT '여행정보 대분류이름',
  code           VARCHAR(100) NULL     COMMENT '코드번호',
  PRIMARY KEY (id)
) COMMENT '여행정보 유형 대분류';
...
(
  ...
  name            VARCHAR(100) NULL     COMMENT '여행정보 중분류이름',
  code            VARCHAR(100) NULL     COMMENT '코드번호',
  PRIMARY KEY (id)
) COMMENT '여행 정보 유형 중분류';

ALTER TABLE travel_middle_class
  ADD CONSTRAINT UQ_travel_large_id UNIQUE (travel_large_id);
...
(
  ...
  addr2             VARCHAR(100) NULL     COMMENT '상세주소',
  contentid         VARCHAR(100) NULL     COMMENT '콘텐츠ID',
...
(
  travel_post_id INT     NOT NULL COMMENT '여행 정보 목록 id',
  user_id        INT     NOT NULL COMMENT '회원번호',
  liked          BOOLEAN NULL     DEFAULT false COMMENT '좋아요',
...
(
  ...
  name             VARCHAR(100) NULL     COMMENT '여행정보 소분류이름',
  code             VARCHAR(100) NULL     COMMENT '코드번호',
  PRIMARY KEY (id)
) COMMENT '여행 정보 유형 소분류';

ALTER TABLE travel_small_class
  ADD CONSTRAINT UQ_travel_middle_id UNIQUE (travel_middle_id);
...
(
  ...
  name VARCHAR(100) NULL     COMMENT '타입 이름',
  code VARCHAR(100) NULL     COMMENT '코드번호',
  PRIMARY KEY (id)
) COMMENT '여행 타입';
...
(
  ...
  last_call_api_id INT          NOT NULL COMMENT 'API호출id',
  areacode_id      INT          NOT NULL COMMENT '지역id',
  basedate         VARCHAR(100) NULL     COMMENT '발표일자',
  basetime         VARCHAR(100) NULL     COMMENT '발표시각',
  fcstdate         VARCHAR(100) NULL     COMMENT '예보일자',
  fcsttime         VARCHAR(100) NULL     COMMENT '예보시각',
  tmn              VARCHAR(100) NULL     COMMENT '최저 기온',
  tmx              VARCHAR(100) NULL     COMMENT '최고 기온',
  sky              VARCHAR(100) NULL     COMMENT '하늘상태',
  pop              VARCHAR(100) NULL     COMMENT '강수확률',
  pty              VARCHAR(100) NULL     COMMENT '강수형태',
  PRIMARY KEY (id)
...
(
  ...
  last_call_api_id INT          NOT NULL COMMENT 'API호출id',
  areacode_id      INT          NOT NULL COMMENT '지역id',
  rnSt4Pm          VARCHAR(100) NULL     COMMENT '4일 후 오후 강수 확률',
  rnSt5Pm          VARCHAR(100) NULL     COMMENT '5일 후 오후 강수 확률',
  rnSt6Pm          VARCHAR(100) NULL     COMMENT '6일 후 오후 강수 확률',
  rnSt7Pm          VARCHAR(100) NULL     COMMENT '7일 후 오후 강수 확률',
  wf4Pm            VARCHAR(100) NULL     COMMENT '4일 후 오후 하늘상태',
  wf5Pm            VARCHAR(100) NULL     COMMENT '5일 후 오후 하늘상태',
  wf6Pm            VARCHAR(100) NULL     COMMENT '6일 후 오후 하늘상태',
  wf7Pm            VARCHAR(100) NULL     COMMENT '7일 후 오후 하늘상태',
  tmFc             VARCHAR(100) NULL     COMMENT '발표시각 ',
  PRIMARY KEY (id)
) COMMENT '중기날씨 중기육상예보';
...
(
  ...
  areacode_id      INT          NOT NULL COMMENT '지역id',
  last_call_api_id INT          NOT NULL COMMENT 'API호출id',
  taMin4           VARCHAR(100) NULL     COMMENT '4일 후 예상최저기온(℃)',
  taMax4           VARCHAR(100) NULL     COMMENT '4일 후 예상최고기온(℃)',
  taMin5           VARCHAR(100) NULL     COMMENT '5일 후 예상최저기온(℃)',
  taMax5           VARCHAR(100) NULL     COMMENT '5일 후 예상최고기온(℃)',
  taMin6           VARCHAR(100) NULL     COMMENT '6일 후 예상최저기온(℃)',
  taMax6           VARCHAR(100) NULL     COMMENT '6일 후 예상최고기온(℃)',
  taMin7           VARCHAR(100) NULL     COMMENT '7일 후 예상최저기온(℃)',
  taMax7           VARCHAR(100) NULL     COMMENT '7일 후 예상최고기온(℃)',
  tmFc             VARCHAR(100) NULL     COMMENT '발표시각 ',
  PRIMARY KEY (id)
) COMMENT '중기날씨 중기기온';
...
ALTER TABLE travelinfo
  ADD CONSTRAINT FK_travel_post_TO_travelinfo
    FOREIGN KEY (travel_post_id)
    REFERENCES travel_post (id);
